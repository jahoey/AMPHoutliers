...
setwd("~/25_MolEcoABC/pop2_0") # This needs to be the directory where the Amarel SFS files are
...
for(i in 1:length(file_list)){
  ...
  full_read <- append(full_read, data_read) # should be 100000 x 255 (pop2_0)
}

# Format to array
data_array <- simplify2array(full_read) #dim should be 15 x 17 x 100000

# Save the data array so that I can load it quickly
save(data_array, file = "~/25_MolEcoABC/pop2_0/data_array.RData")
load(file = "~/25_MolEcoABC/pop2_0/data_array.RData")

#### Downsample all the simulations ####
setwd("~/25_MolEcoABC/pop2_0")
load(file = "~/25_MolEcoABC/pop2_0/data_array.RData")
...
for(i in 1:n_matrices){
  ...
  array.index <- data.frame(Var1 = 1:length(as.list(data_array[,,i]))) # Creating a dataframe with the number of indices equal to the number of entries in a 15 x 17 matrix (255 entries)
...
save(sub.sfs.all2, file = "~/25_MolEcoABC/pop2_0/sub.sfs.all2.RData")
